...
- **設計思想 / 譲れない核**: ファクトは中立・客観。タグは主観。この分離を絶対に崩さない。Leverage スコア計算は仕様書（`metagumo_spec.md` §4）の公式を厳守する。視界切り替え（`flat` / `registered_only` / `leverage`）がコア体験。
- **ターゲットユーザー**: 情報の見え方の差分に関心があるユーザー（第一に作者自身）。
- **現在のフェーズ**: MVP実装中。詳細仕様は [`docs/metagumo_spec.md`](docs/metagumo_spec.md) を参照。フェーズは [`RoadMap.md`](RoadMap.md)、タスクは [`task.md`](task.md) を参照。
...
/packages
  ...
  /types/           # 共有型定義
/docs
  metagumo_spec.md  # 全体仕様書（★最重要。設計の正本）
```
...
- **Leverage 公式の厳守**: `metagumo_spec.md` §4 のスコア計算公式を勝手に変更しない。変更する場合は仕様書を先に更新する。
